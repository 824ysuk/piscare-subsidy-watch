...
def fetch_text(url: str) -> str | None:
    """URL を HTTP GET し HTML テキストを返す。Content-Type charset を優先、fallback は utf-8。"""
...
def detect_changes(
    ...
    - fetch 失敗: state 温存（new_hashes に含めない）、changes に含めない。
    - old_hash 未登録: baseline 確立扱い、changes に含めない（cache 切れによる偽警報を防ぐ）。
    - old_hash != new_hash: changes に追加。
    """
